...
def build_jdbc_options(cfg, watermark_val):
    ...
    # Identifiers left unquoted to match the previous behaviour — Postgres folds
    # unquoted names to lower case, and quoting here would break any entry whose
    ...
    if is_incr and wm_col and watermark_val != '1900-01-01 00:00:00':
        pushdown = f"(SELECT {cols} FROM {schema}.{table} WHERE {wm_col} > '{watermark_val}') AS src"
    elif cols != '*':
        pushdown = f'(SELECT {cols} FROM {schema}.{table}) AS src'
    else:
